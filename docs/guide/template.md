...
你应该看到前面演示创建 `组件代码片段` 模板如 `List.vue` 组件时，可以将 `List.vue` 组件直接拷贝到 `template` 文件夹中，`vue` 组件在缺少相关依赖和运行配置时是没办法和 `html` 文件一样直接预览的，如果某天这个组件模板需要迭代新的功能，而且又交接给另一个人，他看不了组件实际效果没法迭代升级怎么办？接下来就需要解决这个问题。
